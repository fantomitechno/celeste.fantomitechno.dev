...
export const load: PageServerLoad = async () => {
  ...
  const fullClearedMaps = await prisma.map.count({ where: { berries: { equals: prisma.map.fields.berriesGotten, not: 0 } } });

  return { campaignMaps, campaigns, standaloneMaps, totalBerries, goldenBerries, clearedMaps, fullClearedMaps };
};